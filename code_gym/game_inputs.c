...
objects_init(const int screen_height,  const int screen_width, char *buf)
{
    ...
    ball_t ball = (ball_t){
        .radius = 10.0f,
        .active = false,
        .position = (Vector2){ player.position.x + player.size.x/2, player.position.y - ball.radius*2 },
            ...
            if (!state->game_paused) {
                ...
                if (state->objects->ball.active){
                    ...
                    if (((state->objects->ball.position.x + state->objects->ball.radius) >= screen_width) 
                        || ((state->objects->ball.position.x - state->objects->ball.radius) <= 0)) state->objects->ball.speed.y *= -1;
